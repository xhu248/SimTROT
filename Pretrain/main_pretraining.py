...
from utils.synapse_utils import get_loader, patchify_augmentation
...
parser.add_argument('--dataset', default='Task02_Synapse', type=str, help='dataset directory')
parser.add_argument("--json_file", default="jsons/data_btcv.json", type=str, help="directory to the dataset json file")
...
parser.add_argument('--feature_size', default=192, type=int, help='feature size dimention')
...
parser.add_argument('--patch_size', default=8, type=int, help='dimension of patch ')
...
def main_worker(gpu, args):
    ...
    if args.model == "nnformer":
        if 'contrast' in args.pretrain_mode or 'rotation' in args.pretrain_mode:
            model = nnEncoder(pretrain_img_size=(64, 128, 128),
                              window_size=[4, 4, 8, 4],
            ...
            encoder = nnEncoder(pretrain_img_size=(64, 128, 128),
                                window_size=[4, 4, 8, 4],
                                embed_dim=args.feature_size,
                                patch_size=[2, 4, 4],
                                depths=[2, 2, 2, 2],
            ...
            encoder = nnEncoder(pretrain_img_size=(64, 128, 128),
                                ...
                                num_heads=[6, 12, 24, 48],
                                in_chans=args.in_channels)
            model = SimMIM(encoder=encoder, encoder_stride=32, out_channels=(32**3 // 2), mask_patch_size=args.mask_patch_size)
            ...
            encoder = nnEncoder(pretrain_img_size=(64, 128, 128),
                                ...
                                num_heads=[6, 12, 24, 48],
                                in_chans=args.in_channels)
            encoder_k = nnEncoder(pretrain_img_size=(64, 128, 128),
                                window_size=[4, 4, 8, 4],
                                embed_dim=args.feature_size,
                                patch_size=[2, 4, 4],
                                depths=[2, 2, 2, 2],
    ...
    window_size = 64*128*128 / (args.patch_size**3)
