...
from models.unetr import Block, _cfg, PatchEmbed, Patch3DEmbed, get_sinusoid_encoding_table, CNNEncoder
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 use_learnable_pos_emb=False, use_hybrid=False):
        super().__init__()
        self.use_hybrid = use_hybrid
        if use_hybrid:
            self.conv_encoder = CNNEncoder()
            in_chans = 32

        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models

        # change from 2D patchEmbed to 3D patchEmbed
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12,
        ...
        super().__init__()
        self.num_classes = num_classes
        assert num_classes % patch_size ** 3 == 0
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
...
class PretrainVisionTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 in_chans=1,  # avoid the error from create_fn in timm
                 ):
        super().__init__()
